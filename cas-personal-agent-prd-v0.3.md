...
14. **人工修改是权威纠正。** 用户在 Bitable 或外部执行系统中的手动修改优先于旧摘要、模型推断和长期记忆。普通回合读取 SQLite Current State；后台按连接器真实能力吸收变化，只有最新事实、状态不确定、执行或提醒需要时才定向刷新相关对象。
...
| SQLite | 原始事件、版本化 Current State、session 元数据、交接卡、outbox、提醒登记、审计 | 不接管外部执行事实，不做向量搜索 |
...
| 当前状态 | SQLite Current State 为统一本地读模型；飞书多维表格 `项目`、`事项` 与 `行动同步` 为可编辑展示面 |
...
### `current_projects` / `current_items` / `current_action_links`

保存可直接查询和恢复的本地 Current State：稳定 key、外部 record ID、当前字段、递增 revision、来源类型、来源事件、来源发生时间，以及本地请求/待投影/外部确认状态。`current_state_versions` 追加保存每次应用或拒绝的版本、可选基线 revision 与原因。省略字段保留原值，显式 `null` 才清空；旧基线、旧发生时间和重复幂等键不能覆盖较新状态。

## 5.2 飞书多维表格：Current State 展示面
...
- 需要把“想法”升级成外部承诺。
- 普通同事消息、会议邀请或修改已有协同承诺。负责人无法唯一解析的新增协同承诺只问必要问题；负责人唯一且语义明确时可直接创建（ADR-0005）。
...
- 与飞书同事有关、负责人唯一且语义明确的新增分工、交付或协同承诺 → 飞书任务，可直接创建；普通消息、会议邀请和已有协同承诺修改仍遵循单独边界。
...
事实源变化默认由后台按连接器真实能力吸收进 SQLite Current State。连接器没有事件或游标时，使用配置范围内的批量快照差异与定向核对。普通对话不做全量前置读取；用户明确询问最新事实、状态不确定、执行或提醒依赖最新事实时，只刷新相关对象。
...
- 负责人唯一且语义明确的新增协同承诺直接写入飞书任务，并保存负责人、外部 ID 和项目关系。
...
16. **协同任务**：“请小王周五前补接口清单”在小王唯一解析时直接创建飞书任务并保存负责人和外部 ID；歧义时只问一个必要问题。
...
- 失败进入 outbox，重试不重复创建；新增协同承诺遵循 ADR-0005，普通消息、会议邀请与已有承诺修改不随之扩权。
...
- 扩展各连接器经过官方能力验证的事件/游标或范围受限快照同步覆盖。
...
11. 用户在 Bitable、滴答或飞书任务中的人工修改是权威纠正；记忆和 Agent 推断不得覆盖。SQLite Current State 供普通回合读取，后台变化采集与定向刷新按连接器真实能力执行。
